...
export async function POST(req: Request) {
  try {
    ...
    // Tenta pegar do corpo da requisição (Parâmetros de corpo)
    try {
      const body = await req.json();
      nomePaciente = body?.nomePaciente;
    } catch (e) {
      // Ignora erro se não tiver corpo
    }

    // Se não encontrou no corpo, tenta pegar da URL (Parâmetros de consulta)
    if (!nomePaciente) {
